"""
Snapshot of VeeStrategy for revert. Restore with: python scripts/revert_vee.py
State: 1H bias -> 15m CHOCH -> OB+FVG -> entry on return to OB zone (candle in zone); SL beyond OB; TP 3R.
"""
...
class VeeStrategy(BaseStrategy):
    ...
    def prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """Run swing, BOS, FVG on H1 and M15 (M1 only needs OHLC for entry)."""
        swing_len = vc.SWING_LENGTH
        ob_lookback = getattr(vc, "OB_LOOKBACK", 20)
        for name, ref in [("H1", "df_h1"), ("M15", "df_m15")]:
            df = getattr(self, ref)
            ...
            df = detect_fvg(df)
            setattr(self, ref, df)
        if self.df_m1 is not None and not self.df_m1.empty:
    ...
    def run_backtest(self) -> pd.DataFrame:
        ...
        for i in range(20, len(entry_df)):
            ...
            signals.append({
                ...
                "sl": sl,
                "tp": tp,
                "reason": f"Vee: 1H {bias} + 15m CHOCH/OB+FVG, entry on OB zone",
