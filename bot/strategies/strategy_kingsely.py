"""
KingselyStrategy: multi-timeframe smart-money model (1H bias -> 5M setup -> 1M entry).
...
class KingselyStrategy(BaseStrategy):
    """
    ...
    def prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        for name, df_ref in frames:
            ...
            if df is None or df.empty:
                continue
            self._log(f"Kingsely: Detecting swing/BOS/FVG/rejection/displacement on {name}...")
            df = detect_swing_highs_lows(df, swing_length=swing_len)
            df = detect_break_of_structure(df)
            df = detect_fvg(df)
            df = detect_rejection_candle(df, wick_ratio=kc.REJECTION_WICK_RATIO)
    ...
    def detect4HBias(
        ...
        lookback = getattr(kc, "H4_LOOKBACK_BARS", 24)
        ...
        if not getattr(kc, "REQUIRE_4H_ZONE_CONFIRMATION", True):
            return bias, {"zone_type": "BOS_ONLY"}

        wick_pct = kc.REJECTION_WICK_RATIO
        body_pct = kc.REJECTION_BODY_RATIO
        ob_lookback = kc.OB_LOOKBACK
        fvg_lookback = min(kc.FVG_LOOKBACK, len(df) - 1)
    ...
    def detectHTFBias(
        ...
        df = df_h1_slice.tail(kc.HTF_LOOKBACK_HOURS)
        if len(df) < 5:
            return None, None
        ...
        if not getattr(kc, "REQUIRE_HTF_ZONE_CONFIRMATION", True):
            return bias, {"zone_type": "BOS_ONLY"}
        ...
        for i in range(len(df) - 1, max(0, len(df) - kc.FVG_LOOKBACK), -1):
            row = df.iloc[i]
            zone_top, zone_bottom = None, None
            zone_type = None
    ...
    def detectOrderBlock(
        ...
        lookback = ob_lookback or kc.OB_LOOKBACK
    ...
    def checkEntryTrigger(
        ...
        disp_ratio = kc.DISPLACEMENT_RATIO
        ...
        if direction == "BUY":
            ...
            swept, level, _ = self.detectLiquiditySweep(
                df_m1_slice, "BUY", kc.LIQUIDITY_LOOKBACK, end_idx=current_bar_idx + 1
            )
            if swept and bar.get("displacement_bull"):
                return True, float(bar["close"]), "1M sweep low + bullish displacement"
            if getattr(kc, "ALLOW_SIMPLE_ZONE_ENTRY", False) and in_zone and bar["close"] > bar["open"]:
            ...
            swept, level, _ = self.detectLiquiditySweep(
                df_m1_slice, "SELL", kc.LIQUIDITY_LOOKBACK, end_idx=current_bar_idx + 1
            )
    ...
    def manageTrade(
        ...
        """
        if not kc.USE_TRAILING_STOP:
            return None
        daily_limit = balance * (kc.DAILY_LOSS_LIMIT_PCT / 100.0)
    ...
    def run_backtest(self) -> pd.DataFrame:
        ...
        ob_lookback = kc.OB_LOOKBACK
        liq_lookback = kc.LIQUIDITY_LOOKBACK
        min_rr = kc.MIN_RR
        trades_per_session: Dict[str, int] = {}
        trades_per_day: Dict[str, int] = {}
        daily_loss: Dict[str, float] = {}
        max_per_setup = getattr(kc, "KINGSELY_MAX_TRADES_PER_SETUP", None)
        if max_per_setup is None:
            max_per_setup = 1 if getattr(kc, "KINGSELY_ONE_SIGNAL_PER_SETUP", True) else None
        ...
        for i in range(100, len(entry_df)):
            ...
            if apply_limits:
                ...
                if trades_per_session.get(session_key, 0) >= kc.MAX_TRADES_PER_SESSION:
                    continue
            if daily_loss.get(day_key, 0) >= config.INITIAL_BALANCE * (kc.DAILY_LOSS_LIMIT_PCT / 100.0):
                continue

            df_h1_slice = self.df_h1[self.df_h1.index <= idx].tail(kc.HTF_LOOKBACK_HOURS)
            ...
            if getattr(kc, "REQUIRE_BREAKER_BLOCK", False):
                bb = detect_breaker_block(df_h1_slice, bias, ob_lookback=kc.OB_LOOKBACK)
                if bb is None:
                    continue

            if getattr(kc, "REQUIRE_4H_BIAS", False):
                if self.df_h4 is None or self.df_h4.empty:
                    continue
                h4_lookback = getattr(kc, "H4_LOOKBACK_BARS", 24)
                ...
                as_filter = getattr(kc, "H4_AS_FILTER", True)
                if as_filter:
                    ...
                    if bias_4h is None or bias_4h != bias:
                        continue
                if getattr(kc, "BREAKER_BLOCK_4H", False):
                    bb = detect_breaker_block(df_h4_slice, bias, ob_lookback=kc.OB_LOOKBACK)
                    if bb is None:
                        continue

            if getattr(kc, "USE_PREMIUM_DISCOUNT", False):
                eq_lookback = getattr(kc, "EQUILIBRIUM_LOOKBACK", 24)
                eq_tf = getattr(kc, "EQUILIBRIUM_TF", "H1").upper()
            ...
            m5_window = getattr(kc, "M5_WINDOW_HOURS", 12)
            ...
            if getattr(kc, "REQUIRE_5M_SWEEP", True) and not swept:
                continue
            if not swept:
                liq_level = None
            ...
            if entry_zone_top is None:
                ...
                for zt, zb, zd in reversed(zones):
                    if zd == bias:
                        ...
                        break
            if entry_zone_top is None and getattr(kc, "USE_LIQ_LEVEL_AS_ZONE", False) and liq_level is not None:
                ...
                half = atr_val * getattr(kc, "LIQ_ZONE_ATR_MULT", 0.5)
                if bias == "BULLISH":
                    ...
                    entry_zone_top = float(liq_level) + half
                    entry_zone_bottom = float(liq_level) - half
            if entry_zone_top is None and not getattr(kc, "REQUIRE_5M_SWEEP", True):
                bar_row = entry_df.iloc[i]
                atr_val = atr_series.iloc[i] if i < len(atr_series) and atr_series is not None else np.nan
                atr_val = float(atr_val) if not pd.isna(atr_val) and atr_val > 0 else (bar_row["high"] - bar_row["low"]) * 2
            ...
            print(f"[Kingsely] HTF bias detected ({bias})")
            print("[Kingsely] 5M sweep detected")
            print("[Kingsely] 5M BOS detected")
            print("[Kingsely] 1M trigger detected")

            if current_ob is None:
                current_ob = {"high": entry_zone_top, "low": entry_zone_bottom, "midpoint": (entry_zone_top + entry_zone_bottom) / 2}

            sl_method = getattr(kc, "SL_METHOD", "OB")
            sl_atr_mult = getattr(kc, "SL_ATR_MULT", 0.5)
            sl_micro_tf = getattr(kc, "SL_MICRO_TF", "1m")
            ...
            if sl_method == "HYBRID":
                ...
                if base_swing is not None and micro_atr is not None:
                    ...
                    sl_method = "OB"
            if sl_method != "HYBRID":
                sl_buffer = config.get_symbol_config(self.symbol, "KINGSELY_SL_BUFFER") or kc.SL_BUFFER
                buf = sl_buffer * pip
                if bias == "BULLISH":
            ...
            max_spread = config.get_symbol_config(self.symbol, "KINGSELY_MAX_SPREAD_POINTS") or kc.MAX_SPREAD_POINTS
            ...
            if not pd.isna(atr_val) and atr_val > 0:
                ...
                if bar_range > atr_val * kc.MAX_CANDLE_VOLATILITY_ATR_MULT:
                    continue

            if kc.USE_NEWS_FILTER:
                if not is_news_safe(
                    current_time,
                    kc.NEWS_BUFFER_MINUTES,
                    kc.NEWS_BUFFER_MINUTES,
                    True,
            ...
            htf_label = "1H+4H" if getattr(kc, "REQUIRE_4H_BIAS", False) else "1H"
            reason = f"Kingsely: {htf_label} {bias} + 5M setup + {trigger_reason}"
